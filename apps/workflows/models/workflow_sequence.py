...
class WorkflowSequence(BaseModel):
    ...
    def clean(self):
        # Validate that at least one of form_step, approval_step, or action_step is selected
    ...
    def validate_company(self, *args, **kwargs):
        ...
        # validate that worflow and the selected step belong to the same company
        if type == 'Form':
    ...
    def save(self, *args, **kwargs):           
        ...
        # Validate that the workflow and the selected step belong to the same company
        self.validate_company(type=self.type)
